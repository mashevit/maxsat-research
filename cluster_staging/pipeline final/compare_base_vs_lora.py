        ...
        with torch.no_grad():
            ids = model.generate(
                **enc,
                ...
                do_sample=False,
                repetition_penalty=utils.GREEDY["repetition_penalty"],
                temperature=None,
                top_p=None,
                top_k=None,
...
def phase_b(url, model_tag, prompts, max_new_tokens, stops, base_opts):
...
def main():
    ...
    # No getattr default: a missing contract must fail loudly, not degrade to {}.
    greedy = dict(utils.GREEDY)
    stops = greedy.get("stop") or []
    base_opts = dict(greedy["ollama_options"])
    print(f"[contract] ollama options: {base_opts}", flush=True)
    print(f"[contract] stop: {stops!r}", flush=True)
    ...
    b_outs = phase_b(args.ollama_url, args.ollama_model, prompts, args.max_new_tokens, stops, base_opts)
