...
def format_example(example):
    """
    Convert a GSM8K row into prompt-completion format.

    SFTTrainer with completion_only_loss=True expects two columns:
      - prompt:     the input the model conditions on (loss is masked here)
      - completion: the target the model learns to generate (loss applied)
    """
...
def main():
    ...
    # Map into prompt/completion format and drop original columns so SFTTrainer
    # unambiguously picks up the prompt-completion schema.
    train_data = train_data.map(format_example, remove_columns=["question", "answer"])
    val_data = val_data.map(format_example, remove_columns=["question", "answer"])

    print(f"  Train: {len(train_data)}  |  Val: {len(val_data)}  |  Test: held out\n")

    print("--- Sample training example ---")
    print("PROMPT:")
    print(train_data[0]["prompt"][:400])
    print("\nCOMPLETION:")
    print(train_data[0]["completion"][:300])
    print("---\n")
    ...
    # ── 4. Training config (SFTConfig replaces TrainingArguments) ──────────────
    # SFTConfig inherits everything from TrainingArguments and adds SFT-specific
    # knobs. The two we care about:
    #   - completion_only_loss=True: masks loss on the prompt portion.
    #     Replaces the now-removed DataCollatorForCompletionOnlyLM.
    #     The model wastes capacity learning to reproduce
    #     "Solve the following math problem..." without this.
    #   - max_seq_length: moved here from SFTTrainer in newer trl versions.
    #
    # warmup_steps replaces the deprecated warmup_ratio (removed in
    # transformers v5.2). Computing it from total steps preserves the
    # original 5% behavior across both --quick and full runs:
    #   --quick (13 steps)     → 1 warmup step  (effectively no warmup)
    #   full run (~1344 steps) → ~67 warmup steps
    effective_batch_size = args.batch_size * args.grad_accum
    steps_per_epoch = max(1, len(train_data) // effective_batch_size)
    ...
    # ── 5. Train ───────────────────────────────────────────────────────────────
    # No data_collator argument — completion_only_loss in SFTConfig handles
    # the prompt-vs-completion masking that DataCollatorForCompletionOnlyLM
    # used to do.
    trainer = SFTTrainer(
        model=model,
