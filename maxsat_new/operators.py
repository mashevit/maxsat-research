"""EA operators for the memetic solver: selection, crossover, mutation, polish.

Ported from src/evo/operators.py @ 1e3eaaf.

Behavior copied, not text. Ports the four live EA entry points reached from
run_memetic — `tournament`, `clause_aware_crossover1`, `mutate1`, `short_polish` —
plus the one internal helper they reach, `_soft_proxy_scores`. Added type
annotations to public signatures.

Dropped per PORT_NOTES §3 and the Phase-1 reachability audit:
  - `clause_aware_crossover` (v0): superseded by `clause_aware_crossover1`; only
    active caller is the dead `memetic0.py`, not the live path.
  - `mutate` (v0): only active caller is dead `memetic0.py`; commented out in
    memetic.py:92.
  - `short_polish1` (stub): zero call sites anywhere.
  - `_repair_hard_constraints`: only caller is the dropped v0 crossover.
  - `frozen_hard_unit_vars`: unreachable from the four entry points (only active
    caller is the dropped v0 crossover:50; memetic.py:59 has it commented out as
    "#not needed"). Dropped per the Phase-2 reachability rule; see STEP_03_NOTES.
  - all debug `print()` / commented `#print` cruft (none active in ported funcs).

No import from src/. `walksat_polish` comes from the already-ported maxsat_new.walksat.
"""
from __future__ import annotations
from typing import List, Dict, Any, Optional, Tuple
import random
from .population import Individual, evaluate_assignment, clause_satisfied
from .walksat import walksat_polish
...
def _soft_proxy_scores(wcnf: Any) -> Tuple[List[float], List[float]]:
    """
...
def clause_aware_crossover1(
    ...
    def eval_candidate(v: int, val: bool) -> Tuple[int, int]:
        ...
        # Negative occurrences: literal is not x_v
        for cid in neg_occ[v]:
            if hard_satisfied[cid]:
                continue

            if not val:  # x_v = False => not x_v is True
                delta_satisfied += 1
            else:
                if hard_unassigned[cid] == 1:
                    delta_violated += 1
    ...
    def commit_assignment(v: int, val: bool) -> None:
        ...
        for cid in neg_occ[v]:
            ...
            if not val:  # x_v = False => not x_v True
                hard_satisfied[cid] = True
                hard_unassigned[cid] -= 1
            else:
                hard_unassigned[cid] -= 1
...
def mutate1(assign01: List[bool],
    ...
    PORT_NOTES §9.1 (replicated, NOT fixed): `hard_satisfied` is caller-supplied
    and mutated in place here; it is never recomputed inside this function. On the
    live path the caller (memetic, step 7) passes a stale, shared list (a leaked
    init-eval loop variable's `ind.hard_satisfied`), not the current child's — so
    this guard operates against the wrong assignment. Keeping the parameter
    caller-supplied preserves that behavior bit-for-bit; do not compute it here.
    """
    n = len(assign01) - 1
...
def short_polish(
    ...
    wcnf: Any,
    rng_seed: int,
    polish_flips: Optional[int] = None,
    ...
    Signature converted from src's `ls_cfg: dict` to explicit keyword parameters
    (PORT_NOTES §4 resolved-params surface). Defaults are byte-identical to the src
    `ls_cfg.get(...)` defaults; see STEP_03_NOTES §"Deviations" for the full
    src-lookup -> parameter mapping table.
    """
    ...
    res = walksat_polish(
        ...
        rng_seed=rng_seed,
        max_flips=polish_flips,   # §9.2: src looked up "ls_polish_flips" then shadowed "max_flips"; the shadowed fallback key is dropped
        time_limit_s=time_limit_s,  # §9.3: src looked up "time_limit_s" twice (same key, same 0.05 default); written once
        noise=noise,
        hard_safe=hard_safe,
