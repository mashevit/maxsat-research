"""GA population + individual for the memetic EA.

Ported from src/evo/population.py @ 1e3eaaf.

Behavior copied, not text. Whole-module port (PORT_NOTES §3: "this file is lean;
drop nothing"). Added type annotations to public signatures; module docstring
records the port source per the porting rule.

No import from src/. Standalone; the WCNF argument is duck-typed (never imported).
"""
...
# Expect a WCNF-like object (see maxsat_new.cnf.WCNF); passed in, never imported.
...
def jw_priors(wcnf: Any) -> List[float]:
    """
...
def evaluate_assignment(wcnf: Any, assign01: List[bool]) -> Tuple[float, int]:
    """
...
def clause_satisfied(clause: Any, assign01: List[bool]) -> bool:
    """
...
def init_hard_satisfied(hard_clauses: List[Any], assign01: List[bool]) -> List[bool]:
    return [clause_satisfied(cl, assign01) for cl in hard_clauses]


def build_hard_occurs(hard_clauses: List[Any], n_vars: int) -> List[List[int]]:
    """
...
class Population:
    ...
    def init_seeds(self, wcnf: Any, cfg: Dict[str, Any]) -> None:
        pri = jw_priors(wcnf)
        cnt = self.size
        for _ in range(cnt):
    ...
    def evaluate(self, wcnf: Any, ind: Individual) -> Individual:
